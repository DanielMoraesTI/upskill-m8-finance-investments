import { TUpdateTransactionRequest, TTransaction, TCreateTransaction, TransactionSchema, TransactionListResponseSchema, TTransactionList, UpdateTransactionRequestSchema } from "../schemas/transactionSchema";
...
export async function getTransactionList(): Promise<TTransactionList> {
  try {
    ...
    const response = await fetch(`${API_URL}/portal/transactions`, { method: "GET", headers: { Authorization: token } });
...
export async function updateTransaction(id: number, transactionData: TTransaction): Promise<void> {
  try {
    const body: TUpdateTransactionRequest = {
...
export async function deleteTransaction(id: number): Promise<void> {
  try {
    const token = await getUserToken();
    const response = await fetch(`${API_URL}/portal/transactions/${id}`, { method: "DELETE", headers: { "Authorization": token } });
