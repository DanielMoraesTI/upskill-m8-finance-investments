...
async function getAssetByTypeAndTicker(
    ...
    try {
        const rows = await assetRepository.findAssetByTypeAndTicker(assetTypeId, ticker);
        if (rows.length === 0) return null;
        ...
        if (!parsed.success) {
            console.log("Asset parsing error:", parsed.error);
            throw new Error("Invalid asset data");
        }
...
export async function updateAssetCurrentPrice(
    ...
    try {
        const result = await assetRepository.updateAssetCurrentPrice(assetId, newPrice);
